...
export default function ParentJoinPage() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/parent/join", {
        method: "POST",
      ...
      toast.success(
        `Successfully linked to ${data.child?.name || "your child"}!`
      );
        ...
        <div className="card w-full max-w-md bg-base-100 shadow-xl">
          <div className="card-body text-center">
            ...
            <p className="text-sm text-base-content/50 mt-4">
              Have an invite code? Sign in first, then return here.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card w-full max-w-md bg-base-100 shadow-xl">
        <div className="card-body">
          <div className="text-center mb-4">
            <h1 className="text-2xl font-bold">Connect to Your Child</h1>
          ...
          <p className="text-xs text-center text-base-content/50 mt-4">
            The invite code should have been provided by your child&apos;s
            school or coach. Contact them if you don&apos;t have one.
          </p>
        </div>
      </div>
    </div>
